...
lib_path = os.path.join(os.path.dirname(__file__), "build", "main")
...
class LaCAM3InferenceConfig(AlgoBase):
    name: Literal["LaCAM3"] = "LaCAM3"
    time_limit: float = 30
...
class LaCAM3Lib:
    def __init__(self, config: LaCAM3InferenceConfig):
        self.config = config
...
class LaCAM3Inference:
    def __init__(self, config: LaCAM3InferenceConfig, env=None):
        self.config = config
        self.lacam_lib = LaCAM3Lib(config)
    ...
    def act(
        ...
        if self.output_data is None:
            if not self.timed_out:
                self.output_data = self.lacam_lib.run_lacam3(self.env)
                if self.output_data is None:
                    self.timed_out = True
