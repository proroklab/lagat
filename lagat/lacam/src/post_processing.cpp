  ...
  for (size_t t = 1; t < solution.size(); ++t) {
    for (size_t i = 0; i < ins.N; ++i) {
      ...
      for (size_t j = i + 1; j < ins.N; ++j) {
        ...
        // vertex conflicts
        if (v_j_to == v_i_to) {
          info(1, "vertex conflict between agent-", i, " and agent-", j,
               " at vertex-", v_i_to->id, " at timestep ", t);
          return false;
        }
        // swap conflicts
        if (v_j_to == v_i_from && v_j_from == v_i_to) {
          info(1, "edge conflict");
          return false;
        }
      }
    }
                 ...
                 const double comp_time_ms)
{
  ...
  info(1, "solved", "\tcomp_time_ms: ", comp_time_ms,
       " (wo/model load: ", comp_time_ms - MODEL_LOAD_MS, "ms)",
              ...
              const std::string &map_name, const int seed, const bool log_short)
{
  ...
  log << "comp_time_ms_wo_model_load=" << comp_time_ms - MODEL_LOAD_MS << "\n";
  log << "seed=" << seed << "\n";
  if (log_short) return;
  log << "starts=";
  for (size_t i = 0; i < ins.N; ++i) {
